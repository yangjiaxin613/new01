...
//            printf("%c",mqtt_txbuf[j]);
			//1.printf("Sent MQTT CONNECT packet, waiting for response...\r\n");
        for(j=0;j<10;j++)
        {
					//HAL_Delay(50) 时间太短了，要给足响应时间
            HAL_Delay(500);
            if (esp8266_wait_receive() == ESP8266_EOK)
                esp8266_copy_rxdata((char *)mqtt_rxbuf);
						
						
//int len = esp8266_copy_rxdata((char *)mqtt_rxbuf);
...
//    printf("\r\n");
		...
		//printf("%s,%d",mqtt_txbuf,mqtt_txlen);
